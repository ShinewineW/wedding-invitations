...
export class LineEngine {
  ...
  /**
   * First frame only. Each strand is parked off its own side of the screen with
   * its trailing end furthest out, so the spring draws it onto the page from one
   * end rather than fading it in — the first thing the visitor sees is the line
   * being drawn.
   */
  private seed() {
    ...
    for (let s = 0; s < 2; s++) {
      ...
      for (let i = 0; i < this.n; i++) {
        const off = side * this.w * 1.15 * (0.55 + 0.9 * i * inv)
        this.x[s][i] = this.tx[s][i] + off
        this.y[s][i] = this.ty[s][i]
        this.px[s][i] = this.x[s][i] + off * 0.02
        this.py[s][i] = this.y[s][i]
      }
    }
    this.seeded = true
  }
  ...
  private updateTargets() {
    ...
    const cB: ShapeCtx = { ...cA, p: 0 }
    const A = SHAPES[d.chapter]
    const B = SHAPES[d.next]
    const b = d.blend
    const inv = 1 / (this.n - 1)
    for (let s = 0; s < 2; s++) {
      for (let i = 0; i < this.n; i++) {
  ...
  private integrate() {
    ...
    for (let s = 0; s < 2; s++) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        fx += (SX[a] + SX[b] - 2 * cx) * tension
        fy += (SY[a] + SY[b] - 2 * cy) * tension
  ...
  private draw() {
    ...
    const knotAmt = d.chapter === 'tie' ? 1 - d.blend : d.next === 'tie' ? d.blend : 0
    ...
    if (knotAmt > 0.001) {
      for (let s = 0; s < 2; s++) cord(ctx, this.x[s], this.y[s], f.weight, colour[s], inkOpacity)
      return
    }
    ...
    // the one-off moment the thread is first uncovered, reported to the overture
    if (!this.found && !this.reduced && d.chapter === 'overture') {
      let sum = 0
      for (let s = 0; s < 2; s++) for (let i = 0; i < this.n; i++) sum += this.warm[s][i]
      if (sum / (this.n * 2) > 0.26) this.found = true
    }
  }
