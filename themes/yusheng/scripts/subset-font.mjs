...
const content = (
  await Promise.all(['app', 'components/wedding', 'lib'].map(textIn))
).join('');
const glyphs = [
...
for (const [family, file, license] of families) {
  ...
  await writeFile(`app/assets/${file}.woff2`, bytes);
  if (license) {
  ...
  console.log(
    `${family}: ${glyphs.length} glyphs; ${bytes.length} bytes; app/assets/${file}.woff2`,
  );
}
...
const subset = spawnSync(
  ...
  [
    ...
    `--text=${glyphs}`,
    '--flavor=woff2',
    '--output-file=app/assets/letter-hand.woff2',
    '--layout-features=*',
    '--no-recalc-timestamp',
    '--recommended-glyphs',
  ],
...
console.log(
  `LXGW WenKai GB v1.522: ${glyphs.length} glyphs; ${(await readFile('app/assets/letter-hand.woff2')).length} bytes`,
);
