...
for name,body in files.items():
    if name.startswith('wedding2/') or (not a.protected_only and name.startswith('wedding/')):
        path=name[:-10] if name.endswith('/index.html') else name
        items.append((a.origin.rstrip('/')+'/'+path,200,sha(body)))
if not a.protected_only:
    items += [(origin+'/wedding/',200,sha(files['wedding/index.html'])) for origin in a.alias]
