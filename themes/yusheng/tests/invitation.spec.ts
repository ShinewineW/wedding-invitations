...
test('book cover opens as one intact sheet and waits for the guest to continue', async ({
  ...
  await expect(page.locator('#photo-0 .story-photograph')).toBeInViewport();
  expect(await page.locator('.story').count()).toBe(4);
  expect(
    await page.locator('.paper-gesture, .paper-world, [role="slider"]').count(),
  ).toBe(0);
});

test('phone can finish by tapping the fixed arrow without swiping', async ({
  page,
}) => {
  ...
  for (let i = 1; i < 4; i++) {
    ...
    await expect(
      page.locator(`#photo-${i} .story-photograph`),
    ).toBeInViewport();
  }
  await next.click();
  ...
  await next.click();
  await expect(page.locator('#beginning')).toBeFocused();
});

test('four restored notes have different paper designs, fonts and continuous reading', async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto('./');
  ...
  const kinds = ['letter', 'memo', 'postcard', 'book'];
...
test('closing the fourth letter keeps the next step at heartfelt even when its edge is visible', async ({
  page,
}) => {
  ...
  for (const reducedMotion of ['no-preference', 'reduce'] as const) {
    ...
    await expect(page.locator('#heartfelt')).toBeFocused();
    await expect(page.locator('#heartfelt-title')).toBeInViewport();
  }
});
...
test('small screens keep cover facts, every note and controls readable', async ({
  ...
  for (const viewport of [
    ...
    await expect(
      page.getByRole('button', { name: '收好这封信', exact: true }),
    ).toBeInViewport();
    await expect(
      page.getByRole('button', { name: '收起照片背面的信' }),
    ).toBeInViewport();
...
test('full venue, lunar date, schedule and image save action remain consistent', async ({
  ...
  await expect(page.locator('.schedule')).toContainText('17:28');
  await expect(page.locator('.schedule')).toContainText('17:58');
...
test('wedding mount keeps root free and all resource requests in its directory', async ({
  ...
  expect(paths.every((path) => path?.startsWith('/wedding/'))).toBe(true);
