...
const subscribe = () => () => {};
const serverPlatform = () => 'web' as const;
...
export default function MapDirections() {
  ...
  const baidu =
    platform === 'ios'
      ? `baidumap://map/place/search?${baiduQuery}&src=ios.wedding.invitation`
      : platform === 'android'
        ? `bdapp://map/place/search?${baiduQuery}&src=andr.wedding.invitation`
        : baiduWeb;

  return (
    <div className="map-directions">
      <a className="button button-line" href={`${amap}&callnative=1`}>
        查看地点{' '}
      ...
      <details className="map-options">
        ...
        <nav aria-label="选择婚礼地点地图">
          <div className="map-option">
            <a href={`${amap}&callnative=1`}>
              高德地图 <span>默认</span>
            </a>
            <a
              href={`${amap}&callnative=0`}
              target="_blank"
              rel="noreferrer"
              aria-label="高德地图网页版"
            >
          ...
          <div className="map-option">
            <a href={`https://maps.apple.com/?q=${query}`}>
              苹果地图 <ArrowIcon />
            </a>
          </div>
        </nav>
        <p>手机可唤起已安装的地图；若未打开，请用网页版查看。</p>
      </details>
    </div>
  );
}
