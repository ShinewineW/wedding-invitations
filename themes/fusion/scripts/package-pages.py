"""Replace wedding2/ in a complete Pages archive, preserving every other path."""
import argparse
import hashlib
import json
...
manifest = {
    ...
    'wedding2Files': {name: digest(data) for name, data in files.items()
                     if name.startswith('wedding2/')},
}
(project / 'release/package-manifest.json').write_text(
    json.dumps(manifest, indent=2) + '\n')
print(f'Replaced wedding2/; preserved {len(preserved)} files, including wedding/ and all shared controls.')
